            ...
            <form v-for="setting in settings" class="form-vertical" v-show="!loading">
                <div class="card-body px-0" v-show="!loading">
                    <div>
                        <div class="d-flex">
                            <h5>{{setting.name}}</h5> 
                            <label v-if="setting.enabled!=null && setting.name != 'SMS' && setting.name != 'Payment'" class="switch switch-pill switch-outline-success-alt block ml-4">
                ...
                <div v-for="setting_val in setting.settings" class="form-group row justify-content-md-center" v-if="is_setting_visible(setting_val.name)">
                    ...
                    <div class="col-12" v-if="setting_val.value_type == 'text' && is_setting_visible(setting_val.name)">
                        ...
                        type="text" :readonly="setting.enabled==false && setting.name != 'SMS' && setting.name != 'Payment'" v-model="setting_val.value" :placeholder="'Enter ' + setting_val.name">
                    ...
                    <div class="col-12" v-else-if="setting_val.value_type == 'select'">
                        ...
                        <multiselect
                                v-if="setting_val.name == 'Payment Gateway'"
                        ...
                        <multiselect
                                v-if="setting_val.name == 'Paypal Environment'"
                                v-model="paypal_env_value"
                                placeholder="Select Paypal environment"
                                label="value"
                                track-by="value"
                                class="form-control"
                                :options="paypal_env_options"
                                :option-height="104"
                                :show-labels="false">
                            <template slot="singleLabel" slot-scope="props">
                                <div class="option_value">
                        ...
                        <multiselect
                                v-if="setting_val.name == 'Flutterwave Environment'"
                                v-model="flutterwave_env_value"
                                placeholder="Select Flutterwave environment"
                                label="value"
                                track-by="value"
                                class="form-control"
                                :options="flutterwave_env_options"
                                :option-height="104"
                                :show-labels="false">
                            <template slot="singleLabel" slot-scope="props">
                                <div class="option_value">
                        ...
                        <multiselect
                                v-if="setting_val.name == 'Paytabs Region'"
                                v-model="paytabs_region_value"
                                placeholder="Select Paytabs Region"
                                label="value"
                                track-by="value"
                                class="form-control"
                                :options="paytabs_region_options"
                                :option-height="104"
                                :show-labels="false">
                            <template slot="singleLabel" slot-scope="props">
                                <div class="option_value">
...
export default {
    data() {
        return {
            ...
            flutterwave_env_value : {},
            flutterwave_env_options : [
                {value:"staging", label:"Test"},
                {value:"live", label:"Production"},
            ],

            paytabs_region_value : {},
            paytabs_region_options : [
            ...
            payment_options : [
                {value:"none", label:"Not configured"},
                {value:"paytabs", label:"Paytabs", url:"https://site.paytabs.com"},
                {value:"stripe", label:"Stripe", url:"https://stripe.com"},
                {value:"razorpay", label:"Razorpay", url:"https://razorpay.com"},
                {value:"flutterwave", label:"Flutterwave", url:"https://flutterwave.com"},
            ],
            currency_options: [
    ...
    watch: {
        ...
        payment_value: function (val) {
            var payment_setting = this.settings[4].settings.find(item => {
                return item.name === "Payment Gateway";
            });
            if(val == null)
            {
                payment_setting.value = val;
            }
            else
            {
                payment_setting.value = val.value;
            }
        },
        
        paypal_env_value: function (val) {
            var paypal_env_setting = this.settings[4].settings.find(item => {
                return item.name === "Paypal Environment";
            });
            if(val == null)
            {
                paypal_env_setting.value = val;
            }
            else
            {
                paypal_env_setting.value = val.value;
            }
        },

        flutterwave_env_value: function (val) {
            var flutterwave_env_setting = this.settings[4].settings.find(item => {
                return item.name === "Flutterwave Environment";
            });
            if(val == null)
            {
                flutterwave_env_setting.value = val;
            }
            else
            {
                flutterwave_env_setting.value = val.value;
            }
        },
        paytabs_region_value: function (val) {
            var paytabs_region_setting = this.settings[4].settings.find(item => {
                return item.name === "Paytabs Region";
            });
            console.log(paytabs_region_setting);
            if(val == null)
            {
                paytabs_region_setting.value = val;
            }
            else
            {
                paytabs_region_setting.value = val.value;
            }
        }
    },
    methods: {
        getSettings() {
            ...
            axios.get(`/api/settings/getSettings`)
                .then(response => {
                    ...
                    this.updateSMS();
                    this.updatePayment();
                    this.updateFlutterwaveEnv();
                    this.updatePaytabsRegion();
                    this.updatePaypalEnv();
        ...
        is_payment_setting: function(val) {
            return this.payment_options.find(item => {
                return val.indexOf(item.label) !== -1;
            });
        },
        is_selected_payment_setting: function(val) {
            if(this.payment_value)
            {
                return this.payment_options.find(item => {
                    return val.indexOf(this.payment_value.label) !== -1;
                });
            }
        },
        ...
        is_setting_visible: function(val) {
            ...
            else if(this.is_payment_setting(val))
            {
                return this.is_selected_payment_setting(val);
            }
            else
                return true;
        },
        ...
        updatePaypalEnv()
        {
            ...
            this.paypal_env_value =  this.paypal_env_options.find(item => {
                return item.value === paypal_env_setting.value;
            });
        },
        updateFlutterwaveEnv()
        {
            var flutterwave_env_setting = this.settings[4].settings.find(item => {
                return item.name === "Flutterwave Environment";
            });
            this.flutterwave_env_value =  this.flutterwave_env_options.find(item => {
                return item.value === flutterwave_env_setting.value;
            });
        },
        updatePaytabsRegion()
        {
            ...
            this.paytabs_region_value =  this.paytabs_region_options.find(item => {
                return item.value === paytabs_region_setting.value;
            });
        },        
        ...
        updatePayment()
        {
            ...
            this.payment_value =  this.payment_options.find(item => {
                return item.value === payment_setting.value;
            });
        },
        updateSettings() {
            if (!this.submiting) {
